...
class ImageCardCell: UITableViewCell {
    ...
    func setupUI() {
        ...
        if cardVM.isMultiFaces {
            ...
            let url = cardVM.isFlip ? URL(string: cardVM.cardFaces[0].imageUris.normal) : URL(string: cardVM.cardFaces[1].imageUris.normal)
            if let url = url {
                image.kf.setImage(with: url)
            }
        } else {
            flipButton.isHidden = true
            if let url = URL(string: cardVM.imageURLs[0]) {
                image.kf.setImage(with: url)
            }
        }
    }
